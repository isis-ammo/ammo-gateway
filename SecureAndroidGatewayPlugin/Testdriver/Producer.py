from ammo import SecureAndroidConnector
import uuid
import sys
import time
import datetime
...
if __name__ == "__main__":
  ...
  connector = SecureAndroidConnector.SecureAndroidConnector(options.gateway, options.port, deviceName, userName, "")
  connector.setMessageQueueEnabled(False)
  connector.registerMessageCallback(onDataReceived)
  
  scope = SecureAndroidConnector.MessageScope.GLOBAL
  if options.scope == "local":
    scope = SecureAndroidConnector.MessageScope.LOCAL
