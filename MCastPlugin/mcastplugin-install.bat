...
rem **************************************************************************
rem Install AMMO MCastPlugin as a Windows service
rem
...
echo Installing AMMO MCastPlugin ...
...
if not defined CurVer (
  ...
  echo Skipping AMMO MCastPlugin installation
  echo You must install Java before running mcastplugin-install.bat
  exit /b 1
)
...
JavaService -install "AMMO MCast Plugin" "%JVM_DLL%" ^
    -Djava.net.preferIPv4Stack=true ^
    "-Djava.class.path=%PLUGIN_BASE%\gatewaypluginapi.jar;%PLUGIN_BASE%\slf4j-api-1.6.4.jar;%PLUGIN_BASE%\slf4j-simple-1.6.4.jar;%PLUGIN_BASE%\json-20090211.jar;%PLUGIN_BASE%\jgroups-gw.jar;%PLUGIN_BASE%\protobuf-java-2.3.0.jar;%PLUGIN_BASE%\mcastplugin.jar" ^
    -start edu.vu.isis.ammo.mcastplugin.MCastPlugin ^
    -depends "GatewayCore" ^
    -description "AMMO MCast Plugin" 
