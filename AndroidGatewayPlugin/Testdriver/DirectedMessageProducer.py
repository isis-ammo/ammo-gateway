...
if __name__ == "__main__":
  ...
  userName = "user:test/DirectedMessageProducer"
  ...
  try:
    ...
    print "Sending a lot of directed messages."
    sequenceNumber = 0
    
    while True:
      postTime = time.time()
      formattedTime = "{0:.6f}".format(postTime)
      connector.directedMessage("item:" + str(sequenceNumber), "user:test/DirectedMessageConsumer", "application/vnd.edu.vu.isis.ammo.test.TestData", str(sequenceNumber) + "/" + formattedTime, scope)
      print "Pushed directed message number", sequenceNumber, "at", "{0:.6f}".format(time.time())
